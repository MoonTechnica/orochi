...
| `chat/host.rs` (new) | `orochi host --thread <id>`: `chat::run_with` with a `Keyboard` fed from the store rather than from stdin. Built 2026-09-21 after a first version with a turn loop of its own, which seated one agent where a terminal seated two and ran no phase split, no classification pass-through, no memory or feedback — D2 was not true of it. There is one turn loop now, so it cannot drift again |
