...
| What a session costs | The heuristic size prior (`estimated_context + 1500 × scope`) models the work and not the seat, so it under-estimated a real trivial task by about 12× — nearly all of a small session is the system prompt and the cache it reads. `storage::session_floor` measures that floor per (agent, model) from recorded executions and it opens `initial_tokens`, so `expected_tokens` is a real token figure rather than an arbitrary scale; the cache discount still leaves it alone. Because the EWMA learns `measured / predicted`, `Prediction::prior_basis` records how the prior was built and a run predicted another way trains success and latency but not tokens. On 2026-09-20 the same trivial task cost 202,348 tokens on a frontier session and 21,882 on a small one — a 9.2× gap the policy tiers price at 2.46×. The 18 recorded real executions bear this out: on one stratum (`implementation`/normal, scope 4, context ~6,043) `codex/gpt-5.6-luna` measured 23,941–26,606 tokens against `claude/haiku`'s 145,906–175,270 — **6.3× apart at the identical policy price**, both being `small` at 0.65 — so a model with no history of its own starts from what its agent's sessions have cost rather than from the tier. **Automated tests only**; the effect on real routing has not been measured, and a seat whose agent has fewer than four recorded runs is priced exactly as before |
| Whether a second seat pays for itself | A second, read-only seat is another whole session, so it roughly doubles a chat turn however small the work turns out to be — and a bare session is most of what small work costs at all (21,882 tokens for a trivial task on 2026-09-20). `roles::worth_seating` holds it back where `storage::typical_tokens` for that task type and complexity is under `roles.min_work_tokens`, and the console says so. The threshold is **Orochi's own heuristic**, about twice a bare session; a store that has measured nothing, and a team the user asked for outright, are never gated. **Automated tests only** |
...
| Effort in the agent's own words | ACP hands over a `thought_level` selector whose values belong to the agent, and often to its provider's server: `codex-acp` builds the option from the app-server's `supportedReasoningEfforts`, `claude-agent-acp` from the SDK's `supportedEffortLevels` plus a `default` row of its own (both read from the adapters installed here, 2026-09-25). A vocabulary no policy names used to leave every complexity on whatever level the session happened to open with, while the cost factor fell to its middle rung for every level — the effort axis stopped moving *and* stopped costing, so no test could see it. `src/effort.rs` places any vocabulary on one ladder of five rungs, a policy's list for a complexity is read as rungs rather than as words, and the same ladder prices the level that was chosen. The canonical words keep the exact factors they had. **Automated tests only**; no CLI installed here advertises a non-canonical vocabulary to verify against |
| Providers as an open set | `Provider` was a closed enum of three and `Registry::validate` demanded exactly three entries, so an ACP agent from a fourth vendor had to claim to be OpenAI, Anthropic or Google — which files its runs in that vendor's learning strata and prices them by that vendor's tiers. A provider is now a name (lowercase ASCII, at most 24 characters, still serialized as the plain string, so stored records read back unchanged); a registry holds one policy per provider and as many providers as somebody wrote policies for; `orochi policy update --from <dir>` installs every `*.json` in the directory rather than three fixed names; and a provider nobody wrote a policy for is priced by `ProviderPolicy::generic`, which is the same ignorance every bundled policy already spells out for a model it does not recognise (its `*` row), with no cache discount, and reports itself as policy v0. **Automated tests only** |
| Usage read from the payload, not from the config | Anthropic's native `input_tokens` excludes cache reads and writes, so they have to be folded back in. That fold was gated on the *configured* provider, so an agent driving Claude through a bridge whose `provider` was set to anything else had its input undercounted — and since `Usage::weighted()` became what ranks one seat against another, an undercounted input is a model that looks cheaper than it is. It now keys on `cache_read_input_tokens` / `cache_creation_input_tokens`, which are Anthropic's own spelling and nobody else's. An OpenAI-shaped payload has neither key, so it is unchanged whatever its provider is set to. **Automated tests only** |
| Capabilities ACP does not advertise | Driving a browser, reaching the open internet and generating an image are not in the ACP schema at all — `promptCapabilities` says only what a *prompt* may carry, which is whether an image can be sent to the agent and not whether one can be asked of it. They came from config flags alone, and every preset had all three false, so one thing nobody wrote down silently excluded every agent from a task the profiler read as needing the web. Three changes: an agent may now declare them itself in `_meta["orochi.dev/capabilities"]` on `initialize`; a requirement nobody claims to meet is dropped, with the reason recorded on every candidate, rather than emptying the field in silence; and the presets claim what the CLI they drive was read to have — `codex` (`web_search`) and `claude` (`WebSearch`, `WebFetch`), read from the binaries installed here on 2026-09-25, and nothing at all for the CLIs that are not installed here. **Automated tests only** |
| What a reported token total covers | The adapters disagree and nothing in a record said which: `claude-agent-acp` accumulates every assistant message into the turn's figure, `codex-acp` reports `tokenUsage.last` — the final model request alone — and keeps the turn's `total` for its own `/status` (identical in 1.10.0 and the latest 1.13.1, read 2026-09-25). Measured here the same day: `codex` executions at a weighted median of 6,244 against a *classification* on the same seat at 24,262, which is one request and so reported whole — work cannot cost a quarter of the question about it. `Usage::requests` now records how many `usage_update` notifications a turn produced (roughly one per model request in both adapters); **no total is altered and nothing is estimated from it**, because a reconstruction would have to invent the cache/output split. Codex token figures remain a lower bound, and the thresholds calibrated on them are skewed until the adapter reports totals. See [What ACP advertises (2026-09-25)](real-validation-20260925.md) |
...
The previous user input and answer in the session are kept in memory, up to about 24 KiB, and used as context for the next prompt. Each prompt runs the normal scheduler. Keeping the full conversation, persistent session/load, model/mode switching, additional workspaces, and image/audio input are not supported. Unsupported capabilities are not advertised, and unsupported input is an error. Up to 128 sessions.

The MCP servers a client passes on `session/new` belong to that session and are attached to the agent Orochi picks for each of its prompts, together with the ones configured under `[mcp]` (a configured server of the same name keeps its own definition). Only the stdio transport is accepted, which is the only one Orochi advertises and the one every ACP agent must support; `http` and `sse` are refused, as is a server named `orochi-mailbox`. Verified with a real Claude session behind the gateway on 2026-09-22, with a client playing the editor ([Real Validation 2026-09-22](real-validation-20260922.md)); not yet from a real editor.
...
## 7. What ACP advertises, and what it does not

Added: 2026-09-25. **Automated tests only.** Three things Orochi reads from an ACP agent had assumed more than the protocol says, each in a way that failed quietly.

### 7.1. The effort axis is the agent's vocabulary

ACP's `thought_level` selector is a list of opaque values. Orochi never invents one, but it used to *recognise* only the words the provider policies name (`minimal`, `low`, `medium`, `high`, `xhigh`, `max`). Where an agent used other words, `ProviderPolicy::reasoning` matched nothing and fell through to the level the session opened with — the same level for every complexity — and the scorer's cost factor fell to its middle value for every level. Both halves were wrong in the direction of "nothing changed", so the effort axis silently did nothing. Anthropic's own guidance is that tuning effort is usually a better lever than switching models, which makes this the axis that mattered most.

`src/effort.rs` is now the one place that reads the axis, for both questions:

- Five rungs, cheapest first. A word is placed by what it says: `none`/`off`/`minimal`/`fast` on the bottom, `low`, then `medium`/`balanced`/`default`/`auto`/`dynamic`, then `high`/`thorough`/`deep`/`extended`, then `xhigh`/`max`/`highest`/`ultra`. Spelling is normalized, so `x-high`, `x_high` and `very high` are one word.
- A value it does not recognise is placed between its recognised neighbours **in the order the agent advertised them**, so a private word beside two known ones lands where the agent put it. When nothing at all is recognised, the advertised order is taken as ascending — that is what both installed adapters do, and an ordering is the only thing left to read.
- A policy's `reasoning_rules` list for a complexity is read as an ordered set of *rungs*. The agent's own words are matched literally first, so a canonical vocabulary behaves exactly as before; otherwise the same rungs are looked for in the agent's words, and if the rung a policy reaches for does not exist here, the nearest one stands in (a tie going to the dearer level: the rung was asked for, and asking for less thought than the policy wanted is the worse way to miss).
- The cost factor comes from the same ladder, so what was priced is what was chosen. A model with no effort axis, and a level no ladder can place, are priced at the middle rung — neither a discount nor a premium nobody measured. The canonical words keep the factors they had before the ladder existed, so no prediction that was already measured against an outcome moves.

`tests/core.rs` (`mod effort`) pins the real vocabularies read from the installed adapters, and pins both halves together: for an agent speaking `fast`/`balanced`/`thorough`, harder work must ask for a different level **and** be predicted to cost more.

### 7.2. A provider is a name

`Provider` is no longer one of three. It is a lowercase ASCII name of at most 24 characters, serialized as the same plain string as before, so every stored record and every generated SQL column reads back unchanged. A registry holds one policy per provider, and any number of providers; `orochi policy update --from <dir>` installs every `*.json` it finds there. A provider with no policy of its own is priced by `ProviderPolicy::generic`: the `*`-row priors that every bundled policy already applies to a model it does not recognise, the canonical effort rungs (which §7.1 translates into the agent's own words), no cache discount, and version 0, which is what a route explanation shows.

This matters beyond tidiness: telling Orochi that a fourth vendor's agent is "anthropic" files its runs in Anthropic's learning strata, prices its tokens by Anthropic's tiers and reads its error text for Anthropic's phrases. An unknown provider's rate limit is now read against every provider's wording, because each of those phrases means a spent account whoever says it — the alternative is a limit classified as an unknown error, which is a cooldown that never happens and a retry that cannot succeed.

### 7.3. What an agent can do beyond the protocol

ACP has no field for browsing, web access or image generation. Orochi therefore reads three sources and treats a silence as a silence:

- `[[agents]]` flags, as before (`browser`, `web`, `image`).
- The agent's own declaration in `_meta["orochi.dev/capabilities"]` on `initialize` — `{"browser": true, "web": true, "image": false}` — in the shape Orochi's `_meta` extensions for usage and quota already use. Unknown keys are ignored, so a later field does not break an older Orochi.
- Nothing. A requirement that **no** agent in the field claims to meet is dropped rather than enforced, and every candidate carries the reason ("no agent here advertises web; asked anyway rather than excluding every agent"). Where somebody does claim it, it is a real gate and the agents that cannot are left out, exactly as before. The console applies the same rule: a message needing something nobody claims stays with the agent holding the conversation instead of being handed to a capable agent that does not exist.

The presets claim only what the CLI they drive was read to have: `codex` and `claude` declare `web` (`web_search` in the Codex binary, `WebSearch`/`WebFetch` in the Claude binary, read on 2026-09-25); `gemini` and `antigravity` are not installed here and so claim nothing.

### 7.4. What a reported total covers

`Usage::requests` is how many `usage_update` notifications the agent sent while the turn ran. It is not part of any total, and nothing is estimated from it. It exists because what a reported total *covers* is the adapter's choice and the two adapters disagree — see [What ACP advertises (2026-09-25)](real-validation-20260925.md) §3 for the source evidence and the measured effect. Reconstructing a turn's total from the per-request updates was considered and rejected: only the totals are in the stream, so the cache-read and output split would have to be invented, and `Usage` is never estimated.
