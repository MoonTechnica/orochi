...
The set of providers is **open**: a registry holds one policy per provider and any number of providers (`orochi policy update --from <dir>` installs every `*.json` there), and `Registry::get` answers for a provider nobody wrote a policy for with `ProviderPolicy::generic` — Orochi's own neutral prior, never part of a `Registry`, so it is not what `validate` requires provenance of. Adding a provider means adding a policy file, never a variant.
...
- `reasoning_rules` is an ordered list of **rungs**, not of words: `ProviderPolicy::reasoning` matches the agent's advertised values literally first, then looks for the same rungs in the agent's own vocabulary through `effort::Ladder`, then the nearest rung. The cost factor for the chosen level comes from that same ladder — change one and you must change the other, or the axis moves without being priced (or is priced without moving), which is how it silently died once.
- Scoring order in `scorer::candidates` matters: capability flags → quota availability → reasoning/mode resolution → `policy.permits` hard constraints → `required_success` floor → cost. The capability step gates only on a capability some agent in the field advertises; see the capability invariant in `CLAUDE.md`. A candidate rejected by a hard constraint must never reappear later via the router, council or bandit.
